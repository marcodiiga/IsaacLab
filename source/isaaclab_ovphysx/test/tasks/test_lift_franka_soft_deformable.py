...
def _configure_soft_lift_ovphysx_smoke():
    ...
    # still depends on unrelated follow-ups: external table assets and
    # articulation Jacobians for IK.
...
def test_lift_franka_soft_task_reads_and_steps_volume_deformable():
    """Instantiate the soft-lift task and step finite deformable observations/rewards."""
    env = None
    try:
        ...
        # The smoke keeps gripper-only actions: 9 joint pos + 9 joint vel + 60 sampled points + 7 target + 1 action.
        ...
        targets[:, :, 3] = 1.0
        targets[0, :, :3] = deformable.data.nodal_pos_w.torch[0] + torch.tensor(
            [0.0, 0.0, 0.03], device=env.unwrapped.device
        )
        targets[0, :, 3] = 0.0
        deformable.write_nodal_kinematic_target_to_sim_index(
            targets, env_ids=torch.tensor([0], device=env.unwrapped.device)
        )
        readback_targets = wp.to_torch(deformable.root_view.get_simulation_nodal_kinematic_targets())
        torch.testing.assert_close(readback_targets, targets, rtol=1e-5, atol=1e-5)
        ...
        assert torch.mean(deformable.data.nodal_pos_w.torch[0, :, 2] - pre_step_pos[0, :, 2]) > 0.01
    finally:
        if env is not None:
            env.close()
